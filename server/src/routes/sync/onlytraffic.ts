  ...
  (async () => {
    ...
    try {
      ...
      const processOrder = (order: any, type: "CPL" | "CPC") => {
        ...
        stats.matched++;
        ordersToSave.push({ tracking_link_id: urlMatch.id, order_id: order.order_id, order_type: type, order_number: order.order_number || null, source: order.source || null, marketer: order.offer_marketer_name || null, offer_id: order.offer_id || null, offer_marketer_uuid: order.offer_marketer_uuid || null, of_account_id: order.of_account_id || null, quantity_ordered: order.quantity_ordered || 0, quantity_delivered: order.quantity_delivered || 0, price_per_unit: parseFloat(type === "CPL" ? (order.price_per_subscriber || 0) : (order.price_per_click || 0)), total_spent: parseFloat(order.total_spent || 0), status: order.status || null, order_created_at: order.created_at || null, order_completed_at: order.completed_at || null, synced_at: new Date() });
