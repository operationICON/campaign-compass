                            ...
                            {columnOrder.visibleOrderedColumns.map(c => {
                              switch (c.id) {
                                ...
                                case "ltv": {
                                  ...
                                  const ltvVal = isAllTime
                                    ? (link.ltvFromTable ?? 0) + (link.crossPollRevenue ?? 0)
                                    : Number(link.revenue || 0);
                                  const hasRecord = link.hasLtvRecord;
                                  const hasLtv = ltvVal > 0;
                                  // For period: show "—" if no snapshot data (revenue is 0 and not all time)
                                      ...
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <span className={showDash ? "text-muted-foreground" : hasLtv ? "text-[hsl(var(--primary))] font-semibold" : "text-muted-foreground"}>
                                            ...
                                            {isAllTime && hasRecord && link.ltvRecord?.is_estimated && <span className="ml-1 px-1 py-0.5 rounded text-[9px] font-bold bg-[hsl(38_92%_50%/0.15)] text-[hsl(38_92%_50%)] leading-none">Est.</span>}
                                          </span>
                                        </TooltipTrigger>
                                        <TooltipContent>{isAllTime ? (hasRecord ? "Revenue generated by fans this campaign acquired" : "No LTV record — showing $0.00") : "Period revenue from daily snapshots"}</TooltipContent>
                                      </Tooltip>
                                    </td>
                                  );
                                }
