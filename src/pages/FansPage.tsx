          ...
          <div className="px-6 py-4 flex flex-col gap-4">
            ...
            {(() => {
              const visibleCampaigns = (allTrackingLinks as any[])
                ...
                .filter((tl: any) => !campaignsSearch || (tl.campaign_name ?? "").toLowerCase().includes(campaignsSearch.toLowerCase()))
                .sort((a: any, b: any) => Number(b.spenders ?? 0) - Number(a.spenders ?? 0));

              return (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Campaigns</h2>
                      <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{visibleCampaigns.length}</span>
                    </div>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="h-8 pl-8 pr-3 rounded-lg border border-border bg-card text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-1 focus:ring-primary w-48"
                      />
                    </div>
                  </div>
                  ...
                  <div className="bg-card border border-border rounded-xl overflow-hidden">
                        ...
                        <tbody>
                          {visibleCampaigns.length === 0 ? (
                            ...
                            return (
                              <tr
                                key={tl.id}
                                className={cn("border-b border-border/30 hover:bg-muted/20 transition-colors cursor-pointer", i % 2 === 0 ? "" : "bg-muted/10")}
                                onClick={() => {
                                  setSpendersCampaignFilter(tl.id);
                                  setSpendersModelFilter(tl.account_id ? [tl.account_id] : []);
                                }}
                                title="Click to filter All Spenders by this campaign"
                              >
                                <td className="px-4 py-3">
                                  <span className="font-medium truncate max-w-[200px] block">{tl.campaign_name || "—"}</span>
                                </td>
                                <td className="px-4 py-3">
                                  {acc ? (
                                    <div className="flex items-center gap-1.5">
                                      {acc.avatar_thumb_url ? (
                                        <img src={acc.avatar_thumb_url} alt="" className="w-5 h-5 rounded-full object-cover flex-shrink-0" />
                                      ) : (
                                      ...
                                      <span className="text-xs text-muted-foreground truncate max-w-[100px]">{acc.display_name}</span>
                                    </div>
                    ...
                    {visibleCampaigns.length > 0 && (
                      <div className="px-4 py-2 border-t border-border/40 text-[10px] text-muted-foreground text-right">
                        Click a row to filter All Spenders by that campaign
                      </div>
                    )}
                  </div>
                </div>
