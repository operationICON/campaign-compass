...
export default function ExpensesTab() {
  ...
  const { data: allLinks = [], isLoading } = useQuery({ queryKey: ["tracking_links"], queryFn: () => fetchTrackingLinks() });
  const { data: accounts = [] } = useQuery({ queryKey: ["accounts"], queryFn: fetchAccounts });

  const linksWithSpend = useMemo(() => allLinks.filter((l: any) => Number(l.cost_total || 0) > 0), [allLinks]);
  ...
  const distinctAccounts = useMemo(() => {
    ...
    linksWithSpend.forEach((l: any) => { const u = l.accounts?.username; if (u) map.set(l.account_id, u); });
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((l: any) => (l.campaign_name || "").toLowerCase().includes(q) || (l.accounts?.username || "").toLowerCase().includes(q));
    }
  ...
  const bySource = useMemo(() => {
    ...
    linksWithSpend.forEach((l: any) => {
      ...
      map[src].campaigns++; map[src].spend += Number(l.cost_total || 0); map[src].ltv += Number(l.revenue || 0); map[src].profit += Number(l.profit || 0);
    });
  ...
  const byModel = useMemo(() => {
    ...
    linksWithSpend.forEach((l: any) => {
      ...
      map[aid].campaigns++; map[aid].spend += Number(l.cost_total || 0); map[aid].ltv += Number(l.revenue || 0); map[aid].profit += Number(l.profit || 0);
    });
    return Object.values(map).sort((a, b) => b.profit - a.profit);
  ...
  const SortIcon = ({ col }: { col: SortKey }) => { if (sortKey !== col) return <ChevronDown className="h-3 w-3 opacity-30" />; return sortAsc ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />; };

  const handleClearSpend = async (link: any) => {
    try { await clearTrackingLinkSpend(link.id, link.campaign_id); queryClient.invalidateQueries({ queryKey: ["tracking_links"] }); toast.success("Spend cleared"); setClearConfirmId(null); } catch { toast.error("Failed to clear spend"); }
  };

  const onSpendSaved = () => { setCostSlideIn(null); setAddManuallyOpen(false); queryClient.invalidateQueries({ queryKey: ["tracking_links"] }); queryClient.invalidateQueries({ queryKey: ["ad_spend"] }); toast.success("Spend saved — ROI and Profit updated"); };
  ...
  const filteredWithSpend = useMemo(() => filteredAllLinks.filter((l: any) => Number(l.cost_total || 0) > 0), [filteredAllLinks]);
  ...
  const kpiBestSource = useMemo(() => {
    ...
    filteredWithSpend.forEach((l: any) => {
      ...
      map[src].rev += Number(l.revenue || 0); map[src].spend += Number(l.cost_total || 0);
    });
    let best: { name: string; roi: number } | null = null;
    Object.entries(map).forEach(([name, { rev, spend }]) => { if (spend > 0) { const roi = ((rev - spend) / spend) * 100; if (!best || roi > best.roi) best = { name, roi }; } });
    return best;
  }, [filteredWithSpend]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-end gap-2">
        <button onClick={() => setAddManuallyOpen(true)} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium border border-primary text-primary rounded-[10px] hover:bg-primary/10 transition-colors">
          <Plus className="h-3.5 w-3.5" /> Add Manually
        </button>
        <button onClick={() => setCsvOpen(true)} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium border border-border text-muted-foreground rounded-[10px] hover:bg-secondary transition-colors">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {activeFilters.map((f, i) => (
            <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-full bg-[hsl(var(--primary)/0.1)] text-primary">
              {f.label}<button onClick={f.clear} className="hover:opacity-70">✕</button>
            </span>
          ))}
          {activeFilters.length > 1 && (<button onClick={() => { setSourceFilter("all"); setAccountFilter("all"); }} className="text-[11px] text-primary hover:underline">Clear all</button>)}
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center"><TrendingUp className="h-4 w-4 text-primary" /></div>
        ...
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center"><Tag className="h-4 w-4 text-primary" /></div>
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Avg CPL</span>
          </div>
          <p className="text-[22px] font-bold text-foreground">{kpiAvgCPL !== null ? fmtC(kpiAvgCPL) : "—"}</p>
          <p className="text-[11px] text-muted-foreground mt-1">Cost per subscriber</p>
        </div>
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center"><Star className="h-4 w-4 text-primary" /></div>
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Best Source</span>
          </div>
          <p className="text-[22px] font-bold text-primary">{kpiBestSource ? kpiBestSource.name : "—"}</p>
          <p className="text-[11px] text-muted-foreground mt-1">{kpiBestSource ? `${kpiBestSource.roi.toLocaleString("en-US", { maximumFractionDigits: 0 })}% ROI` : "No spend data"}</p>
        </div>
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center"><BarChart3 className="h-4 w-4 text-primary" /></div>
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Campaigns Tracked</span>
          </div>
          <p className="text-[22px] font-bold text-foreground">{kpiTrackedCount} <span className="text-[14px] font-normal text-muted-foreground">of {kpiTotalCount.toLocaleString()}</span></p>
          <div className="mt-2"><Progress value={kpiTrackedPct} className="h-1 bg-secondary" /></div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-[16px] shadow-sm overflow-x-auto">
        {filtered.length === 0 && !isLoading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Go to Tracking Links tab to set spend on campaigns.</p>
          </div>
        ) : (
          <table className="w-full text-xs" style={{ tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: 200 }} /><col style={{ width: 140 }} /><col style={{ width: 110 }} /><col style={{ width: 70 }} />
              <col style={{ width: 90 }} /><col style={{ width: 100 }} /><col style={{ width: 100 }} /><col style={{ width: 100 }} />
              <col style={{ width: 80 }} /><col style={{ width: 90 }} /><col style={{ width: 60 }} />
            </colgroup>
            <thead>
              <tr className="bg-secondary/50 border-b border-border">
                {[
                  { key: "campaign_name" as SortKey, label: "Campaign" }, { key: null, label: "Account" }, { key: null, label: "Source" },
                  { key: null, label: "Type" }, { key: null, label: "Cost Value" }, { key: "cost_total" as SortKey, label: "Total Spend" },
                  { key: "revenue" as SortKey, label: "LTV" }, { key: "profit" as SortKey, label: "Profit" },
                  { key: "roi" as SortKey, label: "ROI" }, { key: null, label: "Status" }, { key: null, label: "" },
                ].map((col, i) => (
                  <th key={i} className={`px-3 py-3 text-left text-[10px] font-medium text-muted-foreground uppercase tracking-wider ${col.key ? "cursor-pointer hover:text-foreground" : ""}`}
                    onClick={() => col.key && handleSort(col.key)}>
                    <span className="flex items-center gap-1">{col.label}{col.key && <SortIcon col={col.key} />}</span>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {filtered.map((link: any) => {
                ...
                return (
                  <tr key={link.id} className="border-b border-border/50 hover:bg-[hsl(var(--primary)/0.03)] transition-colors" style={{ height: 52 }}>
                    ...
                    <td className="px-3 py-2">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${SPEND_TYPE_STYLES[link.cost_type] || "bg-secondary text-muted-foreground"}`}>{link.cost_type || "—"}</span>
                    </td>
                    ...
                    <td className="px-3 py-2"><span className={`text-[12px] font-mono font-semibold ${profit >= 0 ? "text-primary" : "text-destructive"}`}>{profit >= 0 ? "+" : ""}{fmtC(profit)}</span></td>
                    <td className="px-3 py-2"><span className={`text-[12px] font-mono font-semibold ${roi >= 0 ? "text-primary" : "text-destructive"}`}>{fmtP(roi)}</span></td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1 justify-end">
                        <button onClick={() => setCostSlideIn(link)} className="p-1 rounded hover:bg-primary/10 text-primary transition-colors" title="Edit spend"><Pencil className="h-3.5 w-3.5" /></button>
                        {clearConfirmId === link.id ? (
                          ...
                          <button onClick={() => setClearConfirmId(link.id)} className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" title="Clear spend"><X className="h-3.5 w-3.5" /></button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {filtered.length > 0 && (<p className="text-[12px] text-muted-foreground">Showing {filtered.length} campaign{filtered.length !== 1 ? "s" : ""} with spend set</p>)}

      {/* Breakdown Panels */}
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-[16px] shadow-sm p-4">
          <h3 className="text-[13px] font-bold text-foreground flex items-center gap-2 mb-3"><span className="w-2 h-2 rounded-full bg-primary" /> Spend by Source</h3>
          {bySource.length === 0 ? (<p className="text-xs text-muted-foreground py-4 text-center">No data yet</p>) : (
        ...
        <div className="bg-card border border-border rounded-[16px] shadow-sm p-4">
          <h3 className="text-[13px] font-bold text-foreground flex items-center gap-2 mb-3"><span className="w-2 h-2 rounded-full bg-primary" /> Spend by Model</h3>
          {byModel.length === 0 ? (<p className="text-xs text-muted-foreground py-4 text-center">No data yet</p>) : (
            <table className="w-full text-xs">
              <thead><tr className="border-b border-border">
                    ...
                    <tr key={i} className="border-b border-border/30 cursor-pointer hover:bg-secondary/30" onClick={() => { const acc = accounts.find((a: any) => (a.username || "") === row.username); if (acc) setAccountFilter(acc.id); }}>
                      <td className="py-2">
                        <div className="flex items-center gap-2">
                          {row.avatar ? (<img src={row.avatar} className="w-5 h-5 rounded-full object-cover" alt="" />) : (
                            <div className={`w-5 h-5 rounded-full ${acColor.bg} flex items-center justify-center`}><span className={`text-[8px] font-bold ${acColor.text}`}>{row.name[0]}</span></div>
                          )}
      ...
      <CsvCostImportModal open={csvOpen} onClose={() => setCsvOpen(false)} onComplete={() => { setCsvOpen(false); queryClient.invalidateQueries({ queryKey: ["tracking_links"] }); }} trackingLinks={allLinks} />
    </div>
  );
}
