...
function LtvPerModelCard({ accounts, filteredAccountIds, fmtC }: { accounts: any[]; filteredAccountIds: Set<string>; fmtC: (v: number) => string }) {
  const [showInfo, setShowInfo] = useState(false);

  // Query fan_attributions counts per account
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative">
            ...
            {showInfo && (
              <div className="absolute left-0 top-full mt-1 z-30 bg-card border border-border rounded-lg p-3 shadow-lg w-[240px]">
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  30D LTV = revenue generated by subscribers who joined in the last 30 days.
                  <br /><br />
                  Requires Fan Sync to be accurate.
                  <br /><br />
                  Currently showing data for models where fan sync has been completed.
                </p>
              </div>
            )}
          </div>
      ...
      <p className="text-[11px] text-muted-foreground italic mb-3">Revenue from new subscribers in last 30 days</p>
      {sorted.length === 0 ? (
        <p className="text-xs text-muted-foreground py-4">No models</p>
      ) : (
        <div className="space-y-2">
          {sorted.map((acc: any) => {
            const hasFanRows = (fanCounts as Record<string, number>)[acc.id] > 0;
            const ltv30d = acc.ltv_last_30d;
            ...
            if (!hasFanRows) {
              ...
              tooltip = "Run Fan Sync to calculate 30D LTV";
            } else if (ltv30d == null || ltv30d === 0) {
              displayValue = "$0.00";
              displayColor = "text-muted-foreground";
              tooltip = "No new subscriber spend in last 30 days";
            } else {
              displayValue = fmtC(ltv30d);
              displayColor = "text-[#0891b2]";
              tooltip = `30D LTV: ${fmtC(ltv30d)}`;
            }

            return (
              <div key={acc.id} className="flex items-center gap-2" title={tooltip}>
                ...
                <span className={`text-[12px] font-bold font-mono shrink-0 ${displayColor}`}>
                  {displayValue}
                </span>
              </div>
            );
